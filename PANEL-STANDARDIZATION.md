...
- Semantic-index health is end-to-end: Charlie's read-only primary must report a
  successful manifest-backed generation, Alpha's normalized
  `~/.local/share/tower/index/` fallback must carry that same generation, and
  Syncthing must be idle with zero required bytes. Unit success, database file
  mtime, or non-empty stale counts alone may never render GREEN.
