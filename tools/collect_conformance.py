...
def apply_impact_policy(checks: list[dict[str, Any]], manifest: dict[str, Any]) -> None:
    """Mark optional-peer checks without suppressing their collection or evidence."""
    informational_hosts = {str(host) for host in manifest.get("informational_hosts", [])}
    for row in checks:
        check_id = str(row.get("id") or "")
        host = str(row.get("host") or "")
        ssh_target = check_id.rsplit(":", 1)[-1] if check_id.startswith("ssh:") else ""
        row["impact"] = (
            "informational"
            if host in informational_hosts or ssh_target in informational_hosts
            else "required"
        )
...
def main() -> int:
    ...
    try:
        ...
        apply_impact_policy(checks, manifest)
    ...
    required_checks = [row for row in checks if row.get("impact") != "informational"]
    informational_checks = [row for row in checks if row.get("impact") == "informational"]
    required_counts = {name: sum(1 for row in required_checks if row["state"] == name)
                       for name in ("ok", "warning", "error", "unknown")}
    informational_counts = {name: sum(1 for row in informational_checks if row["state"] == name)
                            for name in ("ok", "warning", "error", "unknown")}
    overall = "error" if collector_error or required_counts["error"] else (
        "warning" if required_counts["warning"] or required_counts["unknown"] else "ok"
    )
    check_ids = sorted(f"{row.get('id', '')}|{row.get('impact', 'required')}" for row in checks)
    ...
    payload = {
        ...
        "counts": counts, "required_counts": required_counts,
        "informational_counts": informational_counts, "collector_error": collector_error,
