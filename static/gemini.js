(() => {
  ...
  function updateContext() {
    ...
    workerTitle.textContent = `${selectedLabel} ${selectedProviderLabel} worker run`;
    ...
    workerExplainer.textContent = `One scoped headless ${selectedProviderLabel} run using ${providerButton?.dataset.workerModel || 'the pinned worker model'}. The selected worker profile and a durable Nexus run record remain provider-specific.`;
    workerRunsTitle.textContent = `Recent ${selectedProviderLabel} worker runs`;
    providerDetail.textContent = selectedMode === 'strategy'
      ? providerButton?.dataset.strategyDetail || ''
      : providerButton?.dataset.workerDetail || '';
    terminalElement.setAttribute('aria-label', `Interactive ${selectedProviderLabel} strategy terminal`);
    terminalHost.textContent = `${selectedProvider} · ${selectedHost}`;
  ...
  async function refreshStatus() {
    try {
      ...
      data.hosts.forEach((host) => {
        ...
        state.textContent = strategy.running ? `${selectedProviderLabel} strategy live` : 'available';
      });
      if (data.routing) {
        ...
        document.getElementById('strategyRouting').textContent = strategy.ok
          ? `Pinned strategy route: ${strategy.model} · ${strategy.state} · ${strategy.reason}`
          : `${selectedProviderLabel} strategy routing unavailable: ${strategy.error || 'no eligible provider'}`;
        document.getElementById('workerRouting').textContent = worker.ok
          ? `Pinned worker route: ${worker.model} · ${worker.state} · ${worker.reason}`
          : `${selectedProviderLabel} worker routing unavailable: ${worker.error || 'no eligible provider'}`;
      }
