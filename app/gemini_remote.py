...
Codex, or Gemini strategy commands on the three registered hosts.  Worker
launches are bounded, provider-pinned one-shot runs.  User text is always passed
...
PROVIDERS: dict[str, ProviderSpec] = {
    "claude": ProviderSpec(
        "claude", "Claude", "claude", "opus", "sonnet",
        "Opus 5 · native Claude approvals and resumable sessions",
        "Sonnet 5 · bounded Claude Code execution",
        ("opus", "sonnet", "fable"), True,
    ),
    "codex": ProviderSpec(
        "codex", "Codex", "codex", "gpt-5.6-sol", "gpt-5.6-terra",
        "gpt-5.6-sol · isolated Remote Control profile",
        "gpt-5.6-terra · bounded Codex execution",
        ("gpt-5.6-sol",), False,
    ),
    "gemini": ProviderSpec(
        "gemini", "Gemini", "gemini", "gemini-3.1-pro-high",
        "gemini-3.6-flash-high", "Gemini 3.1 Pro High · native approvals",
        "Gemini 3.6 Flash High · bounded fleet-worker profile",
...
def strategy_command(
    ...
    if provider == "claude":
        cli_argv = [
            "env", f"CLAUDE_CONFIG_DIR={spec.home}/.claude-remote-control",
            spec.claude, "--model", provider_spec.strategy_model,
            "--effort", "high",
        ]
    elif provider == "codex":
        cli_argv = [
            "env", f"CODEX_HOME={spec.home}/.codex-remote-control",
            spec.codex, "--model", provider_spec.strategy_model,
            "--cd", spec.home, "--no-alt-screen",
        ]
    else:
        cli_argv = [
            spec.agy, "--agent", "fleet-strategy", "--model",
            provider_spec.strategy_model,
        ]
...
def worker_command(
    ...
    if provider == "claude":
        ...
        cli_argv = [
            "env", f"CODEX_HOME={spec.home}/.codex-worker", spec.codex,
        ...
        cli_argv = [
            spec.agy, "--agent", "fleet-worker", "--model",
