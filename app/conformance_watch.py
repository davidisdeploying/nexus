...
async def _scan_checks(cache: dict[str, Any], now: datetime) -> dict[str, int]:
    ...
    for ch in raw_checks:
        ...
        if not check_id:
            continue
        # Informational peers remain visible in the cache/UI but never create
        # conformance alarms. Omitting them from active_ids also retires any
        # alert watermark left by the former required-host policy.
        if ch.get("impact") == "informational":
            continue
