...
def _spawn_marked(token: str, duration: float = 5.0) -> subprocess.Popen:
    ...
    # Popen returns before the child necessarily completes exec(). The
    # production liveness probe verifies token ownership from /proc/cmdline,
    # so wait briefly for that exact observable state instead of racing the
    # child and intermittently classifying a valid fixture as a reused PID.
    deadline = time.monotonic() + 1.0
    while time.monotonic() < deadline:
        try:
            if token in Path(f"/proc/{process.pid}/cmdline").read_text(
                errors="replace"
            ):
                break
        except OSError:
