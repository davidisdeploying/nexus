...
from .config import FLEET, Node, ProbeKind, active_fleet
from .fleet_nodes import NODES
from .gemini_remote import HOSTS


class FleetNodeRegistryTests(unittest.TestCase):
    def test_planned_offline_node_is_omitted(self) -> None:
        nodes = [
            Node(name="charlie", address="charlie", kinds=[ProbeKind.DISK]),
            Node(name="delta", address="delta", kinds=[ProbeKind.DISK]),
        ]
        self.assertEqual(
            [node.name for node in active_fleet(nodes, frozenset({"charlie"}))],
            ["delta"],
        )
