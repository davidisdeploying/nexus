...
class ImpactPolicyTests(unittest.TestCase):
    def test_mac_host_and_inbound_ssh_checks_are_informational(self):
        checks = [
            {"id": "agents:macbook", "host": "macbook"},
            {"id": "ssh:alpha:macbook", "host": "alpha"},
            {"id": "agents:alpha", "host": "alpha"},
            {"id": "ssh:alpha:charlie", "host": "alpha"},
        ]
        cc.apply_impact_policy(checks, {"informational_hosts": ["macbook"]})
        self.assertEqual([row["impact"] for row in checks], [
            "informational", "informational", "required", "required",
        ])
