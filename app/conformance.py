...
def project_conformance(
    ...
    if error or not data:
        ...
        return {
            ...
            "required_counts": {"ok": 0, "warning": 0, "error": 0, "unknown": 0},
            "informational_counts": {"ok": 0, "warning": 0, "error": 0, "unknown": 0},
            "non_ok_count": 0,
            "informational_non_ok_count": 0,
    ...
    required_raw = [ch for ch in raw_checks if isinstance(ch, dict) and ch.get("impact") != "informational"]
    informational_raw = [ch for ch in raw_checks if isinstance(ch, dict) and ch.get("impact") == "informational"]
    required_total = len(required_raw)
    informational_total = len(informational_raw)
    required_counts = {state: sum(1 for ch in required_raw if ch.get("state") == state)
                       for state in ("ok", "warning", "error", "unknown")}
    informational_counts = {state: sum(1 for ch in informational_raw if ch.get("state") == state)
                            for state in ("ok", "warning", "error", "unknown")}
    ...
    for raw_ch in raw_checks:
        ...
        if raw_ch.get("impact") == "informational":
            enhanced_ch["human_title"] += " · informational peer"
    ...
    ok_cnt = required_counts.get("ok", 0)
    warn_cnt = required_counts.get("warning", 0)
    err_cnt = required_counts.get("error", 0)
    unk_cnt = required_counts.get("unknown", 0)
    non_ok_count = warn_cnt + err_cnt + unk_cnt
    informational_non_ok_count = sum(
        informational_counts.get(state, 0) for state in ("warning", "error", "unknown")
    )

    if required_total > 0 and ok_cnt == required_total and non_ok_count == 0:
        outcome_headline = f"All {required_total} required checks pass"
        ...
        outcome_headline = f"{ok_cnt} of {required_total} required checks pass ({breakdown})"
    else:
        outcome_headline = "No required checks recorded"
    if informational_non_ok_count:
        outcome_headline += (
            f" · {informational_non_ok_count} informational peer check"
            f"{'s' if informational_non_ok_count != 1 else ''} non-OK"
        )
    ...
    return {
        ...
        "informational_counts": informational_counts,
        "non_ok_count": non_ok_count,
        "informational_non_ok_count": informational_non_ok_count,
