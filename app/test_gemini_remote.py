...
class ControlSurfaceTests(unittest.TestCase):
    ...
    def test_control_page_and_legacy_redirect(self) -> None:
        ...
        for label in ("Claude", "Codex", "Gemini", "Strategy", "Worker"):
...
class CommandContractTests(unittest.TestCase):
    def test_public_provider_contract_allowlists_only_live_session_overrides(self) -> None:
        ...
        self.assertEqual(claude["strategy_models"], ["opus", "sonnet", "fable"])
    ...
    def test_gemini_strategy_commands_are_fixed_and_role_pinned(self) -> None:
        ...
        self.assertIn("fleet-strategy", parsed)

    def test_claude_strategy_uses_isolated_profile_and_opus(self) -> None:
        local, cwd = strategy_command("alpha", "claude")
        self.assertEqual(cwd, "/home/david")
        self.assertIn("CLAUDE_CONFIG_DIR=/home/david/.claude-remote-control", local)
        self.assertIn("opus", local)
        self.assertIn("high", local)
        self.assertNotIn("--dangerously-skip-permissions", local)

    def test_codex_strategy_uses_isolated_profile_and_sol(self) -> None:
        remote, cwd = strategy_command("delta", "codex")
        self.assertIsNone(cwd)
        parsed = shlex.split(remote[-1])
        self.assertIn("CODEX_HOME=/home/david/.codex-remote-control", parsed)
    ...
    def test_worker_prompt_is_one_quoted_argv_element(self) -> None:
        ...
        self.assertIn("fleet-worker", parsed)
        self.assertIn("gemini-3.6-flash-high", parsed)
        self.assertIn("--dangerously-skip-permissions", parsed)

    def test_claude_and_codex_workers_use_distinct_worker_profiles(self) -> None:
        ...
        self.assertIn("CODEX_HOME=/home/david/.codex-worker", codex)
    ...
    def test_worker_status_publishes_runtime_model(self) -> None:
        ...
        self.assertEqual(public["model"], "gemini-3.6-flash-high")
...
class WorkerRoutingTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_worker_launch_can_explicitly_pin_codex(self) -> None:
        ...
        self.assertEqual(job.public()["model"], "gpt-5.6-terra")
